    ...
    {          
        ...
        //{
            ...
            */
            ...
            {
                $select = ("SELECT ".$XML_select." FROM XML_".$idCli." WHERE XML_availability = 1 AND XML_id != '$idProdVista' GROUP BY XML_link ORDER BY XML_click_".$dias." DESC LIMIT 24");
                $result = mysqli_query($conDados, $select);

                echo mysqli_error($conDados);
    
                if(mysqli_num_rows ($result) > 0 )
                {
                    while($select = mysqli_fetch_array($result))
                    {
                        if(setOBJ($idWid, $select, $i, $utm))
                        {
                            $i++;
                        }
                    }                    
            ...
            {
                $select = ("SELECT ".$XML_select." FROM XML_".$idCli." WHERE XML_availability = 1 AND  XML_id != '$idProdVista' GROUP BY XML_link ORDER BY XML_venda_".$dias." DESC LIMIT 24");
                $result = mysqli_query($conDados, $select);
            ...
            {
                $select = ("SELECT ".$XML_select." FROM XML_".$idCli." WHERE XML_availability = 1 AND MATCH(XML_type) AGAINST('\"".$categoria."\"' IN BOOLEAN MODE) GROUP BY XML_link ORDER BY XML_venda_".$dias." DESC LIMIT 24");
                $result = mysqli_query($conDados, $select);
            ...
            {
                $prodNome = getProdName($conDados, $idProdVista, $idCli);
                ...
                if($prodNome != '')
                {
                    ...
                    for($i = 0; $i < $vezesRodar; $i++)
                    {   
                        $select = "SELECT ".$XML_select." FROM XML_".$idCli." WHERE XML_id != '$idProdVista' AND XML_availability = '1' AND XML_titulo LIKE '%".$nomeProdQuebra."%' GROUP BY XML_link ORDER BY XML_click_".$dias." DESC LIMIT 24" ;
                        $result = mysqli_query($conDados, $select);
                        ...
                        if(mysqli_num_rows ($result) > 0 )
                        {
                            while($select = mysqli_fetch_array($result))
                            {
                                if(setOBJ($idWid, $select, $cont, $utm))
                                {
                                    $cont++;
                                }
                            }
            ...
            {
                $select = ("SELECT ".$XML_select." FROM XML_".$idCli." WHERE XML_id != '$idProdVista' AND XML_availability = '1' AND XML_desconto != 100 GROUP BY XML_link ORDER BY XML_desconto, XML_click_".$dias." DESC LIMIT 24");
                $result = mysqli_query($conDados, $select);
            ...
            else if($widTipo == 7) // COLLECTION
            {
                ...
                $arrayPChave = explode(',', $collection);
                ...
                if($check != -1)
                {
                    $arrayPChave[$check] = strtoupper($arrayPChave[$check]);
    
                    $select = ("SELECT ".$XML_select." FROM XML_".$idCli." WHERE XML_id != '$idProdVista' AND XML_availability = '1' AND  UPPER(XML_titulo) LIKE '%".$arrayPChave[$check]."%' ORDER BY XML_click_".$dias." LIMIT 24");
                    $result = mysqli_query($conDados, $select);
                    ...
                    for($i = 0; $i < $vezesRodar; $i++)
                    {
                        ...
                        $select = "SELECT ".$XML_select." FROM XML_".$idCli." WHERE XML_id != '$idProdVista' AND XML_availability = '1' AND XML_titulo LIKE '%".$nomeProdQuebra."%' ORDER BY XML_click_".$dias." DESC LIMIT 24" ;
                        $result = mysqli_query($conDados, $select);
                        ...
                        if(mysqli_num_rows ($result) > 0 )
                        {
                            while($select = mysqli_fetch_array($result))
                            {
                                if(setOBJ($idWid, $select, $conDadost, $utm))
                                {
                                    $conDadost++;
                                }
                            }
            ...
            */
            ...
            {
                $pesquisaProdUnico = explode(',', $ofertaID);

                if($pesquisaProdUnico[0] != "undefined")
                {
                    $select = ("SELECT ".$XML_select." FROM XML_".$idCli." WHERE XML_id = '".$pesquisaProdUnico[0]."'");
                    $result = mysqli_query($conDados, $select);
