...
(function(win, doc, rhAnalytics) {
	...
	RoiHero.transaction.getTransactionId = function() {
		return dataLayer[1].transactionId;
	};
	
	RoiHero.transaction.getPrice = function() {
		var items = dataLayer[1].transactionProducts;
		var prices = '';
	...
	RoiHero.util.getPage = function() {
	    ...
	    var pattCheckoutStep = /checkout\/onepage\/success|checkout\/index\/success/i;
	    var pattCart = /\/checkout\/cart/i;
	    var patt404 = /404/i;
	    var pattHome = /^\/$/i;
	    var pattProd = /produto\//i;
	    var pattSearch = /catalogsearch\/result|busca/i

	    if (pattHome.test(win.location.pathname) && RoiHero.util.getParameterByName("q") == null) {
	        pageType = "home";
	    }
	    else if ((pattSearch.test(win.location.pathname) || RoiHero.util.getParameterByName("q") !== null) && (doc.getElementsByClassName("note-msg").length > 0 || doc.getElementsByClassName("error-msg").length > 0 || (doc.getElementById("no_result").length > 0 && doc.getElementById("no_result").style.visibility == "visible"))) {
	        pageType = "search";
	    }
	    ...
	    else if (typeof Product != "undefined" || pattProd.test(win.location.pathname)) {
	        pageType = "product";
	    }
	    else if (!patt404.test(doc.title)) {
	        pageType = "category";
	    }
	...
	RoiHero.product.getId = function() {
		// TODO verificar como realizar esta recuperação
		return dataLayer[1].ecommerce.detail.products.id.toString();
	};

	RoiHero.product.getPrice = function() {
		return RoiHero.util.formatPrice(doc.getElementById('product-price-1').innerHTML);
	};
    ...
    RoiHero.cart.getId = function() {
    	var tableCart = doc.getElementById('shopping-cart-table');
    	var items = tableCart.getElementsByClassName('product-cart-sku');
    	var ids = '';
    	...
    	for(var i = 0; i < items.length; i++) {
    		...
    		ids += items[i].textContent.trim().substring(5);
    	}
    ...
    RoiHero.cart.getPrice = function() {
		var tableCart = doc.getElementById('shopping-cart-table');
		var items = tableCart.getElementsByClassName('product-cart-price');
		var prices = '';
		...
		for(var i = 0; i < items.length; i++) {
			...
			var item = items[i].getElementsByClassName('price');

			for(var j = 0; j < item.length; j++) {
				prices += RoiHero.util.formatPrice(item[j].textContent);
			}
    ...
    RoiHero.cart.getQuantity = function() {
		var tableCart = doc.getElementById('shopping-cart-table');
		var items = tableCart.getElementsByClassName('qty');
		var qtds = '';
