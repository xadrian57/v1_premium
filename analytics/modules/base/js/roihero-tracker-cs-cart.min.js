...
RoiHero.tagflag.Init = function(page) {
	...
	RoiHero.transaction.getTransactionId = function() {
        ...
        dataLayer.forEach(function(value, key) {
            for(var i = 0; i < value.length; i++){
                if(value[i] == "conversion")
                {
                    idTransacao = value[i+1].transaction_id;
                    break;
                }
            }
        });
	...
	RoiHero.util.getPage = function() {
	    ...
	    var pattCart = /carrinho/i;
	    var pattHome = /^\/$/i;
	    var dataLayer = win['dataLayer'];
	    	
	    if (typeof ab__alp == "object") {	    	
        	if (ab__alp.params.dispatch == "categories.view") 
            {	        		
                pageType = "category";
        	}
	    }
        if (typeof dataLayer == "object") {
            dataLayer.forEach(function(value, key) {
                for(var i = 0; i < value.length; i++){
                    if (value[i].content_type == "product") 
                    {                   
                        pageType = "product";
                        break;
                    }
                    else if(value[i] == "conversion")
                    {
	    ...
	    else if (pattCart.test(win.location.pathname)) {
	        pageType = "cart";
	    }
        else {
            pageType = "search";            
	...
	RoiHero.util.getCategory = function() {
		return document.title.split('::').join('-');
	};
	...
	RoiHero.cart.getId = function() {
        ...
        dataLayer.forEach(function(value, key) {
            for(var i = 0; i < value.length; i++){
                if(value[i] == "add_to_cart")
                {
                    items = value[i+1].items;
                    break;
                }
            }
        });
