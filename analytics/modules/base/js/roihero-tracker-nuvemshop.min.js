...
(function(win, doc, rhAnalytics) {
	...
	RoiHero.transaction.getPrice = function() {
		...
		for(var i = 0; i < items.length; i++) {
			...
			prices += RoiHero.util.formatPrice(items[i].unit_price);
		}
	...
	RoiHero.util.getPage = function() {
	    ...
	    if(ls) {
	    	...
	    	// home, product, category e search
	    	if(ls.template) {
	    		return ls.template;
	    	}
	    	...
	    	if(ls.cart) {
	    		...
	    		// se existir o atributo order, então retorna transaction
	    		if(ls.order) {
	    			return 'transaction'
	    		}
	...
	RoiHero.util.getCategory = function() {
		...
		if(ls.category && ls.category.name) {
			return ls.category.name;
		}
    ...
    RoiHero.util.setCookie = function(idProd, idWidget) {
    		var ids = idProd.toString().split(',');
    		var idsWidget = idWidget.toString().split(',');
	    	var _rh_cmds = win['rh_cmds'];
	    	var page = RoiHero.util.getPage();
    ...
    RoiHero.util.formatPriceRemoveCurr = function(price) {
    	if(price) {
	...
	RoiHero.product.getPrice = function() {
		var price = '',
		    obj = doc.getElementById('price_display');
		
		price = obj.innerHTML.trim();
		price = RoiHero.util.formatPriceRemoveCurr(price);
		
		return price;
	};
	
	RoiHero.util.verificarLS = function(numTentativas) {

        numTentativas = numTentativas ? numTentativas : 0;

        if(LS.length > 0 && numTentativas < 6) {

            setTimeout(function() {

            	RoiHero.util.verificarLS(numTentativas + 1);
