...
(function(win, doc, rhAnalytics) {
	...
	RoiHero.transaction.getTransactionId = function() {
		...
		var ecommerce = RoiHero.util.getEcommercePurchaseFromDataLayer();
	...
	RoiHero.transaction.getPrice = function() {
		...
		var dataLayer = win['dataLayer'],
		    ecommerce = RoiHero.util.getEcommercePurchaseFromDataLayer(),
	...
	RoiHero.util.getPage = function() {
	    ...
	    if (dataLayer) {
	    	...
	    	if(dataLayer[0] && dataLayer[0].pageCategory) {
	    		
	    		if(dataLayer[0].pageCategory == 'Home') {
	    			return 'home';
	    		}
	    		
	    		if(dataLayer[0].pageCategory == 'Produto') {
	    			return 'product';
	    		}

	    		if(dataLayer[0].pageCategory == 'catalogo') {
	    			return 'category';
	    		}

	    		if(dataLayer[0].pageCategory == 'Carrinho') {
	    			return 'cart';
	    		}
	    		
	    		if(dataLayer[0].pageCategory == 'Busca') {
	    			return 'search';
	    		}
	    		
	    		if(dataLayer[0].pageCategory == '') {
	    			if(RoiHero.util.getParameterByName('query') != '') {
	    				return 'search';
	    			}
	    		}
	    		...
	    		if(ecommerce && ecommerce.purchase && ecommerce.purchase.actionField && ecommerce.purchase.actionField.id) {
	    			return 'transaction';
	    		}
	    		
	    		// Após um segundo, verifica se já está posicionado no último passo
	    		setTimeout(function() {
	    			
	    			if(RoiHero.util.isStepOrderConfirmation()) {
	    				// Executando a verificação se o fluxo para fechar o pedido terminou
	    				RoiHero.track.verifyOrderDone();
	    			}
	    		}, 1000);
	    	}
	...
	RoiHero.product.getId = function() {
        for(var i = 0; i < dataLayer.length; i++) {
            if(dataLayer[i].idProduct) {
                return dataLayer[i].idProduct;
            }
        }
	};
	...
	RoiHero.product.getPrice = function() {
		var price = '';
		for(var i = 0; i < dataLayer.length; i++) {
            if(dataLayer[i].priceSell) {
                return dataLayer[i].priceSell.toString();
            }
        }
	};
	...
	RoiHero.util.getCheckoutProducts = function() {
		...
		for(var i = 0; i < dataLayer.length; i++) {
            if(dataLayer[i].ecommerce) {
                if(dataLayer[i].ecommerce.checkout) {
        			if(dataLayer[i].ecommerce.checkout.products) {
	...
	RoiHero.util.verificarDataLayer = function(numTentativas) {
        ...
        if(dataLayer.length < 4 && numTentativas < 6) {
    ...
    RoiHero.cart.getId = function() {
        ...
        var items = RoiHero.util.getCheckoutProducts();
