...
var DEFAULT_URL = 'https://roihero.com.br/';
...
(function(win, doc, rhAnalytics) {
	...
	RoiHero.track.Init = function() {
		...
		switch(page) {
			...
			case "cart": 
			    RoiHero.track.cart();
                window['RhSkuProducts'] = RoiHero.cart.getSku();
                window['RhIdProducts'] = win['rh_cmds'][2][3];
                window['RhQtdProducts'] = win['rh_cmds'][2][4];
                window['RhPriceProducts'] = win['rh_cmds'][2][5];
                window['RhTransactionId'] = '';
                window['RhPaymentType'] = '';
			    isTracked = true;
				break;
			...
			case "transaction": 
                ...
                window['RhSkuProducts'] = RoiHero.transaction.getSku();
                window['RhIdProducts'] = win['rh_cmds'][2][3];
                window['RhQtdProducts'] = win['rh_cmds'][2][4];
                window['RhPriceProducts'] = win['rh_cmds'][2][5];
                window['RhTransactionId'] = win['rh_cmds'][2][6];;
                window['RhPaymentType'] = RoiHero.transaction.getPaymentType();
			    isTracked = true;
				break;
        ...
        var arrayCookieSession = decodeURIComponent(RoiHero.cookie.get('rh_s')).split('|||');
        for(var i = 0; i < arrayCookieSession.length; i++) {
            var item = arrayCookieSession[i].split('=>');
            if(item[0] == 'sid') {
                window['RhIdSession'] = item[1];
                break;
            }
        }

        var arrayCookieSession = decodeURIComponent(RoiHero.cookie.get('rh_v')).split('|||');
        for(var i = 0; i < arrayCookieSession.length; i++) {
            var item = arrayCookieSession[i].split('=>');
            if(item[0] == 'vid') {
                window['RhIdVisitor'] = item[1];
                break;
            }
        }
        
        if(page == "cart" || page == "transaction")
            RoiHero.track.setDataCartAndPurchase(page);

        RoiHero.scraping.Init();
	};

RoiHero.scraping.Init = function() {
    var inputList = RoiHero.scraping.getPageInputs();
    RoiHero.scraping.bindInputListeners(inputList);
}

RoiHero.scraping.getPageInputs = function() {
    var inputs =
    document.querySelectorAll(
        'input:not([type="password"]):not([type="radio"]):not([type="checkbox"])'
    ) || [];
...
RoiHero.scraping.saveInputInfo = function(value, inputInformation) {
    if (!inputInformation.includes(value)) {
        inputInformation.push(value);

        if (RoiHero.scraping.isEmail(value)) {
            RoiHero.ajax.get('https://ne.roihero.com.br/set_email_visitor.php', 'email='+value+
                                                                                '&idStore='+window['rhClientId']+
                                                                                '&idVisitor='+window['RhIdVisitor']
                                                                                );
        }
    }
...
RoiHero.scraping.bindInputListeners = function(inputList) {
    var inputInformation = [];
    for (var $input of inputList) {
        $input.addEventListener("change", event => {
            var {value} = event.target;

            inputInformation = RoiHero.scraping.saveInputInfo(value, inputInformation);
        });
...
RoiHero.scraping.isEmail = function(value) {
...
RoiHero.tagflag.Init = function(page) {
    ...
    RoiHero.track.setDataCartAndPurchase = function(page) {
        RoiHero.ajax.get('https://ne.roihero.com.br/set_cart_and_purchase.php', 'idSession='+window['RhIdSession']+
                                                                            '&idStore='+window['rhClientId']+
                                                                            '&idVisitor='+window['RhIdVisitor']+
                                                                            '&idProducts='+window['RhIdProducts']+
                                                                            '&skuProducts='+window['RhSkuProducts']+
                                                                            '&qtdProducts='+window['RhQtdProducts']+
                                                                            '&priceProducts='+window['RhPriceProducts']+
                                                                            '&transactionId='+window['RhTransactionId']+
                                                                            '&paymentType='+window['RhPaymentType']+
                                                                            '&page='+page);
    }
	...
	RoiHero.track.home = function() {
        ...
        var rh_products = url.searchParams.get("rh_products");
        var stringIdProd = "";
        var stringIdWid = "";
        
        // VERIFICA SE CLIENTE VEIO DE RECUPERAÇÃO
        if(rh_products != null)
        {
            // LOADING NA TELA...
            RoiHero.util.screenLoader.setLoader('Aguarde, montando seu carrinho...');

            var ID_GA = (function () {
                try {
                    if (window._gat.bb !== '') {
                        var id = window._gat.bb
                        ga('create', id, 'auto');
                        return id
                    }
                } catch (e) {}
                ...
                if (typeof ga !== 'undefined') {
                    ...
                    if (ga.getAll) {
                        var gaAarr = ga.getAll()[0];

                        if (gaAarr) {
                            var id = gaAarr.get('trackingId');
                            ga('create', id, 'auto');
                            return id
                        } else return ''
                    }
            ...
            // GA EVENT
            ga('send', { hitType: 'event', eventCategory: 'RH-Cart-Recovery-Email', eventAction: 'RH-Click-Comprar', eventLabel: 'RH-Email' })

            // ADD PRODUTOS AO CARRINHO
            vtexjs.checkout.removeAllItems()
            .done(function(orderForm) {
                
                var item = JSON.parse(rh_products);

                vtexjs.checkout.addToCart(item, null, 1)
                  .done(function(orderForm) {
                        setTimeout(window.location.href = window.location.origin + '/checkout/#/cart', 3000)                
                });
            });            
        }
        else
	...
	RoiHero.track.transaction = function() {
        ...
        var idWidget = RoiHero.util.getWidgetId(idProd);
        var transactionId = RoiHero.transaction.getTransactionId();  // quantity_id
        var price = RoiHero.transaction.getPrice();             // values_id
        var quantity = RoiHero.transaction.getQuantity();  // transaction_id
	...
	RoiHero.transaction.getId = function () {
        ...
        var products = RoiHero.util.transactionProducts();
        var ids = '';
    ...
    RoiHero.transaction.getSku = function () {
        var dataLayer = win['dataLayer'];
        var products = RoiHero.util.transactionProducts();
        var skus = '';
        for (var i = 0; i < products.length; i++){
    ...
    RoiHero.transaction.getPaymentType = function () {
        var dataLayer = win['dataLayer'];
        var paymentTypeFields = RoiHero.util.transactionPaymentType();
        var paymentType = paymentTypeFields.paymentSystemName;
        return paymentType;
    };
	...
	RoiHero.transaction.getTransactionId = function() {
		var dataLayer = win['dataLayer'];
		var ecommerce = RoiHero.util.getEcommerceFromDataLayer();
		var idTransacao = ecommerce.purchase.actionField.id;
		return idTransacao;
    };
    
    RoiHero.util.screenLoader = {
        setLoader: function(text) {
            var css = '#--rPlM{position:fixed;left:0;right:0;bottom:0;top:0;background-color:rgba(0,0,0,.8);z-index:9999999999999999999999999999999999999999999999;font-family:arial,verdana,sans-serif}#--rPlM p.M{color:#fff;text-align:center;font-size:18px;display:block;position:relative;top:40%}.roi-hero-scroll-block{overflow:hidden!important}.lds-ellipsis{display:block;position:relative;width:64px;height:64px;margin:auto;top:37%}.lds-ellipsis div{position:absolute;top:27px;width:11px;height:11px;border-radius:50%;background:#fff;animation-timing-function:cubic-bezier(0,1,1,0)}.lds-ellipsis div:nth-child(1){left:6px;animation:lds-ellipsis1 .6s infinite}.lds-ellipsis div:nth-child(2){left:6px;animation:lds-ellipsis2 .6s infinite}.lds-ellipsis div:nth-child(3){left:26px;animation:lds-ellipsis2 .6s infinite}.lds-ellipsis div:nth-child(4){left:45px;animation:lds-ellipsis3 .6s infinite}@keyframes lds-ellipsis1{0%{transform:scale(0)}100%{transform:scale(1)}}@keyframes lds-ellipsis3{0%{transform:scale(1)}100%{transform:scale(0)}}@keyframes lds-ellipsis2{0%{transform:translate(0,0)}100%{transform:translate(19px,0)}}';
            css = '<style>'+css+'</style>';
    
            var loader = 
            '<div id="--rPlM">'+
                '<p class="--rPlM M">'+text+'</p>'+
                '<div class="lds-ellipsis"><div></div><div></div><div></div><div></div></div>'+
                css+
            '</div>';       
    
            var div = document.createElement('div');
            div.innerHTML = (loader+css);
    
            loader = div.firstChild;
    
            document.body.classList.add('roi-hero-scroll-block');
    
            if (!document.querySelector('#--rPlM'))
                document.body.append(loader);
        },
    
        stopLoader: function() {
            document.body.classList.remove('roi-hero-scroll-block');
            if (document.querySelector('#--rPlM'))
                document.querySelector('#--rPlM').remove()
        }
    }

	RoiHero.util.getPage = function() {
	    var pageType = "unknown";
	    var pattCheckoutStep = /^\/checkout\/orderPlaced/i;
	    var pattCart = /\#\/cart/i;
	    var pattHome = /^\/$/i;
	    var pattSearch = /busca|buscavazia/i
	    var dataLayer = win['dataLayer'];
	    	
	    if (typeof dataLayer == "object") {
	    	dataLayer.forEach(function(value, key) {
	        	if (/Category|Department/i.test(value.pageCategory)) 
                {	        		
                    pageType = "category";
	        	}
                else if(value.pageCategory == 'buscavazia')
                {
                    pageType = "searchEmpty";
                }
	        	else if(value.event && value.event == 'internalSiteSearchView') {
	        		pageType = "search";
	        	}
	        });
	    }
	    if (pattHome.test(win.location.pathname)) {
	        pageType = "home";
	    }
	    else if (pattCart.test(win.location.hash)) {
	        pageType = "cart";
	    }
	    else if (pattCheckoutStep.test(win.location.pathname)) {
	        pageType = "transaction";
	    }
	    else if (typeof win['skuJson'] != "undefined") {
	        pageType = "product";
	    }
	...
	RoiHero.util.getCategory = function() {
		return dataLayer[0].categoryName;
	};
    ...
    RoiHero.util.setCookie = function(idProd, idWidget) {
    	...
    	for(var i = 0; i < ids.length; i++) {
    		rhAnalytics.util.setCookie('rh.'+ids[i], escape("idcli="+rhClientId+" date="+rhDate+" event="+page+" rhWid="+idsWidget[i]), 14, '/');
    	}
    };
    ...
    RoiHero.util.formatPrice = function(price) {
    	if(price) {
    		return RoiHero.util.replaceAll(price, '.', '').replace('R$', '').replace('r$', '').replace(',', '.').trim();
    	}
    	return '';
    };
	...
	RoiHero.product.getId = function() {
        ...
        for(var i = 0; i < dataLayer.length; i++) {
            if(dataLayer[i].productId) {
                return dataLayer[i].productId;
            }
        }
	};
	...
	RoiHero.product.getPrice = function() {
		for(var i = 0; i < dataLayer.length; i++) {
            if(dataLayer[i].productPriceTo) {
                return dataLayer[i].productPriceTo.toString();
            }
        }
	};
	
	RoiHero.util.getEcommerceFromDataLayer = function() {
		var dataLayer = win['dataLayer'];
		for(var i = 0; i < dataLayer.length; i++) {
	...
	RoiHero.util.getOrderFormProducts = function() {
		var dataLayer = win['dataLayer'];
		for(var i = 0; i < dataLayer.length; i++) {
			if(dataLayer[i].orderFormProducts) {
				return dataLayer[i].orderFormProducts;
			}
		}
	...
	RoiHero.util.transactionProducts = function() {
		var dataLayer = win['dataLayer'];
		for(var i = 0; i < dataLayer.length; i++) {
			if(dataLayer[i].transactionProducts) {
				return dataLayer[i].transactionProducts;
			}
		}
    ...
    RoiHero.util.transactionPaymentType = function() {
        var dataLayer = win['dataLayer'];
        for(var i = 0; i < dataLayer.length; i++) {
            if(dataLayer[i].transactionPaymentType) {
                return dataLayer[i].transactionPaymentType[0];
            }
        }
	...
	RoiHero.util.verificarDataLayer = function(numTentativas) {
		...
		var numMaxTentativas = 12;

        numTentativas = numTentativas ? numTentativas : 0;

        if(dataLayer.length < 5 && numTentativas < numMaxTentativas) {
	...
	// GET
	RoiHero.ajax.get = function (url, params, funcSuccess, funcFail) {
	...
	RoiHero.cart.getId = function() {
        var idsProd = '';
        var dataLayer = win['dataLayer'];
        var items = RoiHero.util.getOrderFormProducts();
    ...
    RoiHero.widget.Init = function(page, idProd, category) {
    				...
    				if(obj) {
    					...
    					if(obj.widgets) {
                            ...
                            for(var i = 0; i < obj.widgets.length; i++) {
                                var itemAux = obj.widgets[i];

                                if(itemAux.widget_inteligencia == 44)
                                {
                                    rco = true;
                                }

                                if(itemAux.widget_inteligencia == 13)
                                {
                                    flagOverlay = true;
                                }
                            }
                            
                            if(RoiHero.util.getPage() != 'transaction')
                            {
                                vtexjs.checkout.getOrderForm()
                                .done(function(orderForm) {
                                    var items = orderForm.items;

                                    if (RoiHero.cookie.get('rhRCO') == '' && !doc.getElementById('rhRCO') && rco && items.length > 0) {
                                        flagOverlay = false;
                                        var div = doc.createElement('div');
                                        div.setAttribute("id", "rhRCO");
                                        doc.body.appendChild(div);
                                    }

                                    if (RoiHero.cookie.get('rhOverlay') == '' && !doc.getElementById('rhOverlay') && flagOverlay) {
                                        var div = doc.createElement('div');
    ...
    RoiHero.widget.css = function(idCliente) {
        ...
        var _rh = doc.createElement('link'); _rh.async = true;
        _rh.rel="stylesheet"; _rh.type="text/css"; _rh.href = DEFAULT_URL + 'widget/css/overlay/rh_overlay_'+idCliente+'.css';
        var _rh_s = doc.getElementsByTagName('link')[0]; _rh_s.parentNode.insertBefore(_rh, _rh_s);
    };
