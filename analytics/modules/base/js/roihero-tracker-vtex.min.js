...
RoiHero.tagflag.Init = function(page) {
	...
	RoiHero.track.home = function() {
        ...
        var rh_rec_id = url.searchParams.get("rh_rec_id");
        var rh_rec_wid = url.searchParams.get("rh_rec_wid");
        var rh_products = url.searchParams.get("rh_products");
        var stringIdProd = "";
        var stringIdWid = "";
        
        // VERIFICA SE CLIENTE VEIO DE RECUPERAÇÃO
        if(rh_rec_id != null && rh_products != null)
        {
            // LOADING NA TELA...
            ...
            // EVENTO GA CLIQUE ABRIR OFERTA LIMITADA
            ga('send', { hitType: 'event', eventCategory: 'RH-Recovery', eventAction: 'RH-Click-Comprar', eventLabel: 'RH-EmailRec' })
            ...
            .done(function(orderForm) {
                ...
                vtexjs.checkout.addToCart(item, null, 1)
                  .done(function(orderForm) {
                    ...
                    // CRIAÇÃO DE COOKIE COM O ID
                    for(var i = 0; i < item.length; i++)
                    {
                        RoiHero.cookie.set('rh.'+item[i].id, escape("idcli="+rhClientId+" date="+String(new Date().getTime())+" event=cart rhWid="+rh_rec_wid), [0,14,0,0], "/");                        
                    }

                    // CRIAÇÃO DE COOKIE COM O ID
                    RoiHero.cookie.set("rh_rec_id", rh_rec_id, [0,3,0,0], "/");

                    // AJAX PARA SETAR OPEN
                    RoiHero.ajax.post(
                            'https://re.roihero.com.br/recuperacao.php',
                            'rh_rec_id=' + rh_rec_id,
                            // Success
                            function(data)
                            {
	...
	RoiHero.track.transaction = function() {
        ...
        var rh_rec_id = RoiHero.cookie.get('rh_rec_id');
        ...
        if(rh_rec_id != '')
        {
            ...
            var array_quantity = quantity.split(',');
            var valueTotal = 0;

            for(var i = 0; i < array_price.length; i++)
            {
                valueTotal = valueTotal + (array_price[i] * array_quantity[i]);
            }

            // AJAX PARA SETAR NOVO 
            RoiHero.ajax.post(
                    'https://re.roihero.com.br/recuperacao.php',
                    'rh_rec_id=' + rh_rec_id +
                    '&rh_order_id=' + transactionId +
                    '&rh_value=' + valueTotal,
                    // Success
                    null,
                    //Fail
