...
RoiHero.tagflag.Init = function(page) {
	...
	RoiHero.track.transaction = function() {
		...
		var waitConfirmation = function() {
			setTimeout(()=> {
				...
				var url = doc.URL;
				var page = url.substring(url.length - 12, url.length);

				// Só realiza o processamento se for a página de confirmação
	...
	RoiHero.transaction.getTransactionId = function() {
		...
		for(var i = 0; i < dataLayer.length; i++) {
			if(dataLayer[i].transactionId) {
				idTransacao = dataLayer[i].transactionId;
				break;
			}
		}
	...
	RoiHero.util.getPage = function() {
	    ...
	    var pattSearch = /pesquisa/i;
	    var dataLayer = win['dataLayer'];

        // CONFIGURAÇÃO PARA LOWEL
	    
	    if (dataLayer[0].pageType == "generic") {
	        pageType = "home";
	    }
	    else if (dataLayer[0].pageType == "product") {
	        pageType = "product";
	    }
	    else if (dataLayer[0].pageType == "search") {
	        pageType = "category";
	    }
	    else if (pattSearch.test(doc.URL)) {
	    	pageType = "search";
	    }
	    else if (dataLayer[0].pageType == "cart") {
	        pageType = "cart";
	    }
	    else if (dataLayer[0].pageType == "login") {
	...
	RoiHero.product.getId = function() {
		var idProd = dataLayer[0].products.productId;
		return idProd;
	};

	RoiHero.product.getPrice = function() {
		var price = dataLayer[0].products.productPrice;;
	...
	RoiHero.util.getCartProducts = function() {
		var dataLayer = win['dataLayer'];
		for(var i = 0; i < dataLayer.length; i++) {
			if(dataLayer[i].products) {
				return dataLayer[i].products;
			}
		}
	...
	RoiHero.util.transactionProducts = function() {
		...
		for(var i = 0; i < dataLayer.length; i++) {
			...
			if(dataLayer[i].ecommerce && dataLayer[i].ecommerce.transactionProducts) {
				return dataLayer[i].ecommerce.transactionProducts;
			}
		}
	...
	RoiHero.util.verificarDataLayer = function(numTentativas) {
		...
		var numMaxTentativas = 24;

        numTentativas = numTentativas ? numTentativas : 0;

        if(dataLayer.length < 3 && numTentativas < numMaxTentativas) {
	...
	RoiHero.cart.getId = function() {
        var idsProd = '';
        var dataLayer = win['dataLayer'];
        var items = RoiHero.util.getCartProducts();
        ...
        for (var i = 0;i < items.length; i++) {
            ...
            idsProd += items[i].productId.toString();
        }
    ...
    RoiHero.cart.getPrice = function() {
        ...
        for (var i = 0;i < items.length; i++) {
        	...
        	prices += items[i].productPrice.toString();
        }
