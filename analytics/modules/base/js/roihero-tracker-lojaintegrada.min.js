...
(function(win, doc, rhAnalytics) {
	...
	RoiHero.transaction.getTransactionId = function() {
		// devolve algo como "<i class="icon-check cor-principal icon-fixed-width"></i>5"
		var idTransacao = doc.getElementsByClassName('numero-pedido')[0].innerHTML.split('>');
		return idTransacao.slice(-1)[0].trim();
	};
	
	RoiHero.transaction.getPrice = function() {
		var produtos = doc.getElementsByClassName('produto-info');
		var precos = doc.getElementsByClassName('preco-produto');
		var valores = '';
		for (var i = 0; i < produtos.length; i++){
			
		    var valor = RoiHero.util.formatPrice(precos[i].getElementsByTagName('strong')[0].innerHTML);
	...
	RoiHero.transaction.getQuantity = function () {
		...
		var qtds = '';
		for (var i = 0; i < produtos.length; i++){
			var qtd = produtos[i].parentElement.nextElementSibling.children[0].innerHTML.trim();
	...
	RoiHero.transaction.getId = function () {
		...
		var ids = '';
		for (var i = 0; i < produtos.length; i++){
			var sku = produtos[i].innerHTML.split('<strong>')[1].split('</strong>')[0].trim();
	...
	RoiHero.util.getPage = function() {
		var pageType = "";
	    var patt = /checkout\/[0-9]*\/finalizacao/;

	    if (win.location.pathname == "/") {
	        pageType = "home";
	    }
	    else if( RoiHero.util.isCategoryPage() ) {
	        pageType = "category";
	    }
	    else if( RoiHero.util.isProductPage() ) {
	        pageType = "product";
	    }
	    else if (win.location.pathname.toLowerCase() == "/carrinho/index") {
	        pageType = "cart";
	    }
	    else if (patt.test(win.location.pathname)) {
	...
	/* Foi implementado na lojaintegrada, para auxiliar na verificação se é a página de produto */
	RoiHero.util.isProductPage = function() {
		var metas = doc.getElementsByTagName('meta');
		for(var i = 0; i < metas.length; i++) {
			var meta = metas[i];
			if(meta.getAttribute('name') == 'twitter:card') {
				return meta.getAttribute('content').toLowerCase() == "product";
			}
		}
		return false;
	};
	...
	/* Foi implementado na lojaintegrada, para auxiliar na verificação se é a página de produto */
	RoiHero.util.isCategoryPage = function() {
		return doc.getElementsByClassName("pagina-categoria").length > 0;
	};
    ...
    RoiHero.util.removeOldProdCookies = function(numOldCookies) {
    	...
    	if(maxNumProdCookies == arrayDate.length) {
    		
    		numRealToDelete = numOldCookies;
    		
    	} else if(arrayDate.length > maxNumProdCookies) {
    		
    		numRealToDelete = (arrayDate.length - maxNumProdCookies) + numOldCookies;
    			
    	} else if((arrayDate.length + numOldCookies) < maxNumProdCookies) {
    		numRealToDelete = 0;
    	}
    	
    	for(var i = 0; i < numRealToDelete; i++) {
    		var cookieToExpire = arrayDateCookie[arrayDate[i]];
    		RoiHero.util.deleteCookie(cookieToExpire);
    	}
    };
	...
	RoiHero.product.getId = function() {
		var idProduto = doc.getElementsByClassName('codigo-produto')[0].children[0].children[1].innerHTML;
		return idProduto.toString();
	};

	RoiHero.product.getPrice = function() {
		return RoiHero.util.formatPrice(doc.getElementsByClassName('preco-promocional')[0].innerHTML);
	};

    RoiHero.cart.getId = function() {
        var idsProd = '';
        var skuProdutos = doc.getElementsByClassName('produto-info');

        for (var i = 0;i < skuProdutos.length; i++) {    
            var sku = skuProdutos[i].innerHTML.split('<strong>')[1].split('</strong>')[0].trim();
    ...
    RoiHero.cart.getPrice = function() {
        ...
        for (var i = 0;i < skuProdutos.length; i++) {    
            var preco = RoiHero.util.formatPrice(precos[i].getElementsByTagName('strong')[0].innerHTML);

            if(precoProd == '')
                precoProd = preco;
            else
                precoProd = precoProd+','+preco;
        }
    ...
    RoiHero.cart.getQuantity = function() {
        var qtdProd = '';
        var tabelaProdutos = doc.getElementsByClassName('tabela-carrinho border-alpha')[0];
        var produtosLista = tabelaProdutos.getElementsByTagName('tr');

        var count = 0;
        for (var i = 0; i < produtosLista.length; i++){
            var qtd = produtosLista[i].getAttribute('data-produto-quantidade');
            if (qtd) {
                if(qtdProd == '')
                    qtdProd = qtd;
                else
