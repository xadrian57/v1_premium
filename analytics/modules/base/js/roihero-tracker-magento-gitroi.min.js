...
(function(win, doc, rhAnalytics) {
	...
	RoiHero.transaction.getTransactionId = function() {
		...
		return ecommerce.purchase.actionField.id;
	};
	...
	RoiHero.transaction.getPrice = function() {
		...
		if(ecommerce.products) {
			prds = ecommerce.products;
		} else {
			prds = ecommerce.purchase.products;
		}
	...
	RoiHero.product.getId = function() {
		var ecommerce = RoiHero.util.getEcommerceFromDataLayer();
		try{
			var prds = ecommerce.detail.products
			if(prds instanceof Array) {
				var ids = '';
				for(var i = 0; i < prds.length; i++) {
					if(i > 0) {
						ids += ',';
				...
				return ecommerce.detail.products.id.toString();
			}
		} catch (e) {
		}
		return '';
