...
//Módulo widget
//Realiza as chamadas dos widgets
//@author roihero
//@type objeto
RoiHero.widget = {};
...
(function(win, doc, rhAnalytics) {
	...
	RoiHero.track.Init = function() {
		...
		RoiHero.product.trackProductPage();
	};
	...
	*/
	...
	RoiHero.product.trackProductPage = function() {
		...
		RoiHero.util.getClientJson(url);
	};
	...
	RoiHero.util.getClientJson = function(url) {
		
		//var idCli = win['rhClientId'];
		var idCli = '0132';
		...
		var jsonUrl = DEFAULT_URL + 'dashboard/widgets/search_bar/return_json.php?idcli=' + idCli + '&tipo=0';
		...
		req.onreadystatechange = function()
		{
		    ...
		    if(req.readyState == 4 && req.status == 200) 
		    {
		    	...
		    	RoiHero.util.verifyClientJson(jsonUrl, req);
		    }
		}
		req.send(null);
	};
	
	RoiHero.util.verifyClientJson = function(prdUrl, req) {
		if(req.responseText) {
			var objJson = {
				jsonClient: JSON.parse(req.responseText.trim())
			};
			
			for(var i = 0; i < objJson.jsonClient.length; i++) {
				var item = objJson.jsonClient[i];
				
				// Verificando a url
				if(prdUrl == item.link) {
					
					attr.idProduct = item.id;
					attr.price = item.salesprice;
					
					RoiHero.track.product();
					break;
				}
			}
			delete objJson.jsonClient;
		}
	};
	...
	RoiHero.product.getId = function() {
		return attr.idProduct.toString();
	};
 ...
 // Injeta o html retornado na página do cliente
    RoiHero.widget.injectWidget = function(item) {
		...
		if("id" == item.inject_type) {
			...
			objRef = doc.getElementsByClassName(item.inject_name)[0];
		}
